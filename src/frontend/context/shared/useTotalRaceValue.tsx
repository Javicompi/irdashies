...
export const useTotalRaceValue = () => {
  ...
  // A ref (not state): the projection changes at telemetry rate during a
  // race and writing it to state from the render body produced a setState
  // cascade that exceeded React's nested-update limit (crash #185) once the
  // hook's inputs started updating at mixed rates (60 Hz telemetry, 5 Hz
  // channel snapshots). Re-renders already happen on every telemetry tick,
  // so consumers always see the fresh value.
  const lastLeaderRaceLapsRef = useRef(0);
  ...
  const bestLapTime = useTelemetryValues('CarIdxBestLapTime')?.[carIdx] as
    number | undefined;
  ...
  const leaderAvgLapTime =
    avgLapTimes[leaderLapTimeIdx] > 0
      ...
      : (classEstLapTimes?.[leaderLapTimeIdx] ?? 0) > 0 &&
          classEstLapTimes?.[leaderLapTimeIdx] !== undefined
  ...
  const playerAvgLapTime =
    avgLapTimes[carIdx] > 0
      ? avgLapTimes[carIdx]
      : (classEstLapTimes?.[carIdx] ?? 0) > 0 &&
          classEstLapTimes?.[carIdx] !== undefined
  ...
  } else if (lastLeaderRaceLapsRef.current > 0) {
    result.leaderRaceLaps = lastLeaderRaceLapsRef.current;
  }
  ...
  if (isFixedLapRace) {
      ...
      if (lapsValid) {
        ...
        if (totalLeaderDist > totalDist) {
          result.adjustedRaceTime =
            (totalLaps - Math.floor(totalLeaderDist - totalDist)) *
            leaderAvgLapTime;
        }
      }
    }
    ...
    const playerDist =
      lap !== undefined && lap > 0 ? lap - 1 + (lapDistPct ?? 0) : 0;

    const leaderPaceWall =
      elapsed > 0 && leaderDist > 0 ? elapsed / leaderDist : 0;
    const playerPaceWall =
      elapsed > 0 && playerDist > 0 ? elapsed / playerDist : 0;
    ...
    if (
      ...
      const leaderFinalLap = Math.ceil(
        (elapsed + timeRemaining) / leaderPaceWall
      );
      const effectiveRaceTime = leaderFinalLap * leaderPaceWall;
      result.totalRaceLaps =
        Math.round((effectiveRaceTime / playerPaceWall) * 10) / 10;
      ...
      result.totalRaceLaps =
        Math.round((effectiveRaceTime / playerAvgLapTime) * 10) / 10;
    }
  ...
  if (
    ...
    lastLeaderRaceLapsRef.current = result.leaderRaceLaps;
  }
